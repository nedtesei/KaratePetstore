...
public class KarateRunnerTest {
    ...
    private String classpath = "classpath:com/petstore/karate";
    ...
    public void run() throws Exception {
        ...
        Results results = Runner.path(Optional.ofNullable(options.getPaths()).orElse(Arrays.asList(classpath)))
                .hooks(options.createHooks()).tags(options.getTags()).configDir(options.getConfigDir())
                .karateEnv(karateEnv).outputHtmlReport(true).outputCucumberJson(true).outputJunitXml(true)
